...
**Resolution**: Test rewritten to fund tom with TUNA (correct payment) and inject FISH as spam via `additionalPurchaseFee` mock. Payment now contains correct cost token PLUS unexpected extra tokens, passing Check A and hitting the spam rejection in `isRightPayment()`. Error regex updated to `/unexpected tokens in payment/`. Commits 4b8612b, c389e54.
