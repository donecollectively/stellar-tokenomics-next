...
**Status**: Complete — type-checks clean after on-chain enum + bridge regeneration
...
None — resolved. On-chain `WithdrawingProceeds` enum variant added by osiris, bridge regenerated. Build confirms zero MarketSale type errors.

#### Additional Fix (pre-existing)

| Commit | Description |
|--------|-------------|
| `89c60b6` | Fixed TS2322 in `mkTxnActivateMarketSale` — `.validFor()` stripped the `TCX` generic; added `as TCX` cast. Also narrowed generic constraint from `hasCharterRef \| StellarTxnContext` to `StellarTxnContext`. Pre-existing, not introduced by this work unit. |
...
### Onchain Coder (osiris) — 2026-02-24
...
| File | Change | REQTs |
|------|--------|-------|
| `src/MarketSale/MarketSalePolicy.hl` | Added explicit tags to SpendingActivity: `WithdrawingProceeds` (51), `Retiring` (65), `SoldOut` (86) | — |
| `src/MarketSale/MarketSalePolicy.hl` | Added `WithdrawingProceeds` handler: state gate, tokens unchanged, datum unchanged, gov authority | REQT/czp1jhqgdj, ayvw26q6av, aexkjfxm2k, gy6jd9cjkg, ykqx9qgh88, 5r79v9b4ht |
| `src/MarketSale/MarketSaleData.hl` | Added `requireVxfFieldsNone()` on `FixedSaleDetailsV1` | REQT/1h49829nsx, 88cfkdj7p2 |
| `src/MarketSale/MarketSaleData.hl` | Replaced VXF validation in `validateActivating` with `requireVxfFieldsNone()` | REQT/4z5dkz9p2p — supersedes x3xkgwp94t, 0sa1x0aj4z |
| `src/MarketSale/MarketSaleData.hl` | Replaced VXF validation in `validateCommonUpdateChecks` with `requireVxfFieldsNone()` | REQT/1h49829nsx, 88cfkdj7p2 — supersedes g563tb3ks1 |
| `src/MarketSale/MarketSalePolicy.hl` | Replaced VXF validation in `Resuming` with `requireVxfFieldsNone()` | REQT/1h49829nsx, 88cfkdj7p2 — supersedes d67c29t29w, jkbaba8n7n |
| `src/MarketSale/MarketSalePolicy.hl` | Added `requireVxfFieldsNone()` in `SellingTokens`; annotated funds accumulation and token routing | REQT/wh3kjtwmj9, nnxpz49srs |

#### Design Decisions

| Topic | Resolution |
|-------|------------|
| VXF helper scope | `requireVxfFieldsNone()` on `FixedSaleDetailsV1` — the struct that owns the fields. One bREQTgroup covering both fields, called from 4 activities. |
| Explicit enum tags | `Retiring` (65), `SoldOut` (86) regularised; `WithdrawingProceeds` (51) placed below active-sale activities, above lifecycle-terminal ones. |
| REQT-wh3kjtwmj9 / nnxpz49srs | Enforced by comment annotation + existing value logic — no new assertion needed since the behavior is already correct. |
| REQT-5r79v9b4ht | Enforced by absence — no ADA constraint in WithdrawingProceeds handler. |

#### Commits

| Commit | Description |
|--------|-------------|
| b077263 | Topics 1-2: WithdrawingProceeds enum variant + handler |
| f2a9446 | Topics 3-5: VXF None-mode enforcement via requireVxfFieldsNone() |
| de5cb8f | Topics 6-7: SellingTokens VXF enforcement + traceability annotations |
