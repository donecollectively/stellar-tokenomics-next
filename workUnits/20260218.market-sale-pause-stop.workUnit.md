...
Commit `0306f26` changed Stopping/Resuming/Retiring from `input().value == output().value` to `get_assets()` equality + ≤1 ADA tolerance. Driven by minUtxo shifts when datum CBOR size changes. Requirements and acceptance criteria still say "UTxO value unchanged" — need updating.
