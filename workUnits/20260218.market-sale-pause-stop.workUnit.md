...
5. **Extract `validateCommonUpdateChecks`** on `MarketSaleData` — a shared helper called by both `validateUpdatePendingSale` and the new `validateUpdatePausedSale`. Extracts the five invariant checks that apply to ANY sale update regardless of state:
...
6. **Add `UpdatingPausedSale` spending activity** (separate from `UpdatingPendingSale`):
   ```
...
7. **Add `validateDetailsWhenPaused`** to `SaleProgressDetailsV1`:
   - All four fields must equal previous (`lotsSold`, `lastPurchaseAt`, `prevPurchaseAt`, `lotCount`)

8. **Wire up `Retiring`** (currently stubbed) for `Paused → Retired` transition only:
   ```
...
|----------|--------|------|--------------------|
| Architect | Approved | 2026-02-18 | F1 (Retiring scope) and F2 (off-chain plan gap) resolved. Requirements coverage verified — all ARCH-scope behaviors have driving REQTs. |
| Code Whisperer | Approved | 2026-02-18 | F3 (shared validation extraction) resolved — mandated in plan. Pre-work advisory complete; no architectural escalations. |
| Domain-Fit Tester (Stellar Testing) | Approved | 2026-02-18 | F4 (defense-in-depth test gap) resolved. Test sketch in `MarketSale.test.ts` + `MarketSaleTestHelper.ts`: 24 test cases across 4 activities, snapshot chain, helper methods. |
...
#### Stellar Testing — test sketch

Test sketch placed directly in the test file and helper per stakeholder preference:

- **`MarketSale.test.ts`**: 4 `describe` blocks (Stopping, Resuming, UpdatingPausedSale, Retiring) with 24 `it()` blocks — each with stable slug, REQT reference, and TODO-commented body showing real helper calls, assertions, and expectError patterns. Happy-path tests consolidate assertions into single transactions.
- **`MarketSaleTestHelper.ts`**: Snapshot chain (`firstMarketSalePaused` → parent `firstMarketSaleActivated`, `firstMarketSaleResumed` → parent `firstMarketSalePaused`) plus 4 helper methods (`stopMarketSale`, `resumeMarketSale`, `updatePausedMarketSale`, `retireMarketSale`) — each with TODO-commented transaction building using the `mkTxnUpdateRecord` + activity redeemer pattern.
