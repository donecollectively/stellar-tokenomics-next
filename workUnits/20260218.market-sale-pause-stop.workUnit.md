...
**Status**: Draft
**Gaps**:
- No ARCH-* items exist in this project — Architect needs to establish them or confirm scope without them
- No REQT files exist — REQM needs to formalize requirements (the `requirements()` methods in controller code serve as informal requirements currently)
- Domain-fit tester pre-work sign-off needed
...
**ARCH Items**: *(none established yet — gap)*
**REQT Items**: *(none formalized yet — gap; informal requirements live in `MarketSaleController.requirements()` and `MarketSalePolicy.hl` activity handlers)*
...
Proposed:  Pending → Active ⇄ Paused
                  ↘          ↗
                   (both can → Retired)
```
...
7. **Wire up `Retiring`** (currently stubbed) for `Paused → Retired` transition only:
   ...
   - Gov authority required
   - Token withdrawal/burning details (to be further specified)
...
4. **Update `beforeUpdate()`** — currently special-cases Active state; needs Paused handling (no fixups needed — all progress details frozen)
5. **Update `findMarketSales()`** — add `paused?: boolean` filter flag (currently only has `pending`, `active`, `retired`)
6. **Update `requirements()`** — add requirement entries for the new activities
7. **Pre-flight validation** — verify state is Active before Stop, Paused before Resume/Edit/Retire
...
4. **Retiring tests** (from Paused):
   ...
   - Requires gov authority
   - Token withdrawal/burning validated
...
- [ ] `Retiring` spending activity transitions Paused → Retired with gov authority and token withdrawal
