...
- [ ] `Retiring` spending activity transitions Paused → Retired with gov authority; all fields except state unchanged; UTxO token assets unchanged, ADA within 1 ADA tolerance (tokens stay locked for future CleanupRetired)
- [ ] Selling is rejected when state is Paused
- [ ] Frozen fields: state (except via dedicated activities), salePace, all progressDetails, entire saleAssets struct, startAt, threadInfo, id, type, UTxO token assets (ADA within 1 ADA tolerance for minUtxo shifts)
...
| Reviewer | Status | Date | Notes / References |
|----------|--------|------|--------------------|
| Architect | Approved with Notes | 2026-02-19 | pw-arch-f1: UTxO value REQTs updated for ADA tolerance — [post-work-audit.md](./20260218.market-sale-pause-stop.post-work-audit.md) |
| Code Whisperer | Approved with Notes | 2026-02-19 | pw-cw-f1: comment corrected; pw-cw-f2: evolution documented — [post-work-audit.md](./20260218.market-sale-pause-stop.post-work-audit.md) |
| Domain-Fit Tester (Stellar Testing) | Approved with Notes | 2026-02-19 | pw-st-f1: wiring proof added; pw-st-f2: test count corrected — [post-work-audit.md](./20260218.market-sale-pause-stop.post-work-audit.md) |
...
| UTxO token assets | (on-chain value) | Token assets (non-ADA) unchanged; ADA within 1 ADA tolerance for minUtxo shifts from datum CBOR encoding changes |
...
*(Corrected during post-work review: `resume-invalid-name` removed — REQT/fkww59zyt3 is implemented on-chain but untestable via negative test because validateStateOnlyChange catches all mutations first. Coverage via assertEnforcedReqts wiring proof in happy path. Previous count was 31.)*
...
##### Finding pw-arch-f1: UTxO value check relaxed — exact equality → asset equality + ADA tolerance

Commit `0306f26` changed Stopping/Resuming/Retiring from `input().value == output().value` to `get_assets()` equality + ≤1 ADA tolerance. Driven by minUtxo shifts when datum CBOR size changes.

> **Resolution**: 4 REQTs updated (hk93w5zb16, tx3fyv3eb2, 998waf4mz3, dtpwzjqn9p) — "UTxO Value Unchanged" → "UTxO Token Assets Unchanged" with ADA tolerance clause. Acceptance criteria and frozen field inventory updated. Rationale: state enum change alters CBOR encoding size → minUtxo shifts.
...
> **Resolution**: Comment rewritten. Coverage via `assertEnforcedReqts` wiring proof on happy path.

##### Finding pw-cw-f2: Implementation evolution in 0306f26 not captured in worker reports

REQT traceability added to `validate()` (REQT-y16j4t955c, REQT-egttdcamhg) and `validateCommonUpdateChecks` (REQT-ntdbhc1xss, REQT-rg5zyhd2gb, REQT-6z88fg6j2s, REQT-b731sye0fz). Four test error regexes corrected. Positive improvements but undocumented implementation evolution.

> **Resolution**: Documented. No code changes — traceability additions and regex corrections are correct. Stakeholder commit `0306f26` acknowledged in post-work record.

#### Stellar Testing — sequenced follow-up (2 findings)
...
> **Resolution**: Added `assertEnforcedReqts` to Resuming happy path — 6 REQT traces including REQT-fkww59zyt3. Test count 30 confirmed.

##### Finding pw-st-f2: Test count in Advisory Notes says 31, actual is 30

After removal of `resume-invalid-name`, actual: Stopping:6 + Resuming:8 + UpdatingPausedSale:10 + Retiring:6 = 30.

> **Resolution**: Test count summary table corrected to 30. Historical interview/worker report references left as-is.
