...
#### Stopping/Retiring — "all fields unchanged" enforcement approach

Under discussion: serialize-and-compare (`previous.copy(state changed).serialize() == self.serialize()`) vs field-by-field comparison. Serialize approach gives strongest guarantee — any new field added in future is automatically covered. Commented-out code in existing `validateActivating` (lines 257-264) shows this pattern was previously considered.
...
#### Remaining items
- Self-audit against patterns JSONL and essential guide — pending
- Interview complete pending self-audit
