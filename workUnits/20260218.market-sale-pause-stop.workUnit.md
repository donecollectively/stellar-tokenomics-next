...
Shared helper `validateStateOnlyChange(self, previous, ddd, targetState, reqtId)` on `MarketSaleData` checks:
- `name`, `fixedSaleDetails`, `saleAssets`, `threadInfo` (deep `==` against previous)
- `progressDetails`, `salePace` (within saleState, deep `==`)
- UTxO token value unchanged (`ddd.input().value == ddd.output().value`)
- `id`/`type` immutability handled by framework (`mustOutputDelegatedData`) — not re-checked

Used by both Stopping and Retiring. Caller handles state assertions separately (with per-activity REQT traceability).
...
- `validateStateOnlyChange` for all-fields-unchanged + UTxO value (REQT-03ff0mfddc)
